...
class NewReleaseAdapter(private var list: List<GenreDatum>, public var mContext: Context,var listener : RecylerViewItemClickListener)
    : androidx.recyclerview.widget.RecyclerView.Adapter<androidx.recyclerview.widget.RecyclerView.ViewHolder>() {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): androidx.recyclerview.widget.RecyclerView.ViewHolder {
        val inflater = LayoutInflater.from(parent.context)
            return NewReleaseViewHolder(inflater, parent)
    }

    override fun onBindViewHolder(holder: androidx.recyclerview.widget.RecyclerView.ViewHolder, position: Int) {
            val movie: GenreDatum = list[position]
            (holder as NewReleaseViewHolder).bind(movie, mContext)
        if (position == 0){
            Glide.with(mContext)
                    ...
                    .into((holder as NewReleaseViewHolder).iv_image!!)

        }
            (holder as NewReleaseViewHolder).itemView!!.tag = list[position]
            (holder as NewReleaseViewHolder).itemView!!.id = position
            (holder as NewReleaseViewHolder).itemView!!.setOnClickListener(View.OnClickListener {
                val source = it.tag as GenreDatum
                val id = it.id
                    listener.onNewRelase(id, source)
            })
...
class NewReleaseViewHolder(inflater: LayoutInflater, parent: ViewGroup) :
