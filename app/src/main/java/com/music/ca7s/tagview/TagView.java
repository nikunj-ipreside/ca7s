package com.music.ca7s.tagview;
...
public class TagView extends RelativeLayout {
     ...
     * tag list
     */
    private List<TagClass> mTags = new ArrayList<>();
    ...
    private void drawTags() {
        ...
        TagClass tagPre = null;
        for (TagClass item : mTags) {
            final int position = listIndex - 1;
            final TagClass tag = item;
...
//            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN) {
                tagLayout.setBackgroundResource(tag.getIcon());
//            } else {
//                tagLayout.setBackgroundResource(tag.getIcon());
//            }
            ...
            tagView.setTextColor(tag.getColor());
//            tagView.setTextSize(TypedValue.COMPLEX_UNIT_SP, tag.getTagTextSize());
            ...
            tagLayout.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    if (mClickListener != null) {
                        mClickListener.onTagClick(tag, position,mTags);
                    }
                }
            });
            ...
            tagLayout.setOnLongClickListener(new OnLongClickListener() {
                ...
                public boolean onLongClick(View v) {
                    if (mTagLongClickListener != null) {
                        mTagLongClickListener.onTagLongClick(tag, position,mTags);
                    }
                    return true;
                }
            });
            ...
            float tagWidth = tagView.getPaint().measureText(tag.getName()) + textPaddingLeft + textPaddingRight;
...
//                            mDeleteListener.onTagDeleted(TagView.this, "", position);
//                        }
//                    }
//                });
                tagWidth += deletableView.getPaint().measureText(String.valueOf("")) + textPaddingLeft + textPaddingRight;
...
//                    }
    ...
    private Drawable getSelector(Tag tag) {
        ...
        gdNormal.setColor(tag.getLayoutBackgrounf());
    ...
    public void addTags(List<TagClass> tags) {
        if (tags == null) return;
        mTags = new ArrayList<>();
        if (tags.isEmpty())
            drawTags();
        for (TagClass item : tags) {
            mTags.add(item);
        }
        drawTags();
    }
    ...
    public void addTags(TagClass tags) {
//        if (tags == null) return;
//        for (TagClass item : tags) {
...
//        }
    ...
    public interface OnTagClickListener {
        void onTagClick(TagClass tag, int position, List<TagClass> mTags);
    }

    public interface OnTagLongClickListener {
        void onTagLongClick(TagClass tag, int position, List<TagClass> mTags);
    }
}
